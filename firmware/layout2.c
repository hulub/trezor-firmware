...
void layoutHome(void) {
	...
	const char *hulub = "Hello Hulub from ";
	const char *label = storage_isInitialized() ? storage_getLabel() : "-----";

	// Determine new size
	int newSize = strlen(hulub) + strlen(label) + 1;

	// Allocate new buffer
	char * newBuffer = (char *) malloc(newSize);

	// do the copy and concat
	strcpy(newBuffer, hulub);
	strcat(newBuffer, label); // or strncat

	const char *text = newBuffer;
	...
	if (homescreen) {
		...
		if (label && strlen(label) > 0) {
			...
			oledDrawStringCenter(OLED_HEIGHT - 8, text);
		} else {
...
void layoutConfirmOutput(const CoinType *coin, const TxOutputType *out) {
	...
	const char *str_out = str_amount(out->amount,
			coin->has_coin_shortcut ? coin->coin_shortcut : NULL, buf_out,
			sizeof(buf_out));
	layoutDialogSwipe(&bmp_icon_question, "Cancel", "Confirm",
	NULL, "Confirm sending", str_out, "to", first_half, out->address + 17,
	NULL);
}

void layoutConfirmTx(const CoinType *coin, uint64_t amount_out,
		uint64_t amount_fee) {
	const char *str_out = str_amount(amount_out,
			coin->has_coin_shortcut ? coin->coin_shortcut : NULL, buf_out,
			sizeof(buf_out));
	const char *str_fee = str_amount(amount_fee,
			coin->has_coin_shortcut ? coin->coin_shortcut : NULL, buf_fee,
			sizeof(buf_fee));
	layoutDialogSwipe(&bmp_icon_question, "Cancel", "Confirm",
	NULL, "Really send", str_out, "from your wallet?", "Fee included:", str_fee,
	NULL);
}

void layoutFeeOverThreshold(const CoinType *coin, uint64_t fee, uint32_t kb) {
	(void) kb;
	const char *str_out = str_amount(fee,
			coin->has_coin_shortcut ? coin->coin_shortcut : NULL, buf_out,
			sizeof(buf_out));
	layoutDialogSwipe(&bmp_icon_question, "Cancel", "Confirm",
	NULL, "Fee", str_out, "is unexpectedly high.",
	NULL, "Send anyway?",
	NULL);
}
...
void layoutSignMessage(const uint8_t *msg, uint32_t len) {
	...
	layoutDialogSwipe(&bmp_icon_question, "Cancel", "Confirm", "Sign message?",
			str[0], str[1], str[2], str[3], NULL, NULL);
}

void layoutVerifyAddress(const char *address) {
	const char **str = split_message((const uint8_t *) address, strlen(address),
			17);
	layoutDialogSwipe(&bmp_icon_info, "Cancel", "Confirm", "Confirm address?",
			"Message signed by:",
			NULL, str[0], str[1], str[2], NULL);
}

void layoutDisplayMessage(const char *message, const char *description) {
	const char **str = split_message((const uint8_t *) message, strlen(message),
			17);
	layoutDialogSwipe(&bmp_icon_info, "Cancel", "Confirm", description,
	NULL, str[0], str[1], str[2], str[3], str[4]);
}

void layoutDisplayCandidate(const char *candidate_name) {
	const char **str = split_message((const uint8_t *) candidate_name,
			strlen(candidate_name), 17);
	layoutDialogSwipe(&bmp_icon_question, "Next", "Vote",
...
void layoutVerifyMessage(const uint8_t *msg, uint32_t len) {
	const char **str = split_message(msg, len, 16);
	layoutDialogSwipe(&bmp_icon_info, "Cancel", "Confirm", "Verified message",
			str[0], str[1], str[2], str[3], NULL, NULL);
}
...
void layoutEncryptMessage(const uint8_t *msg, uint32_t len, bool signing) {
	const char **str = split_message(msg, len, 16);
	layoutDialogSwipe(&bmp_icon_question, "Cancel", "Confirm",
			signing ? "Encrypt+Sign message?" : "Encrypt message?", str[0],
...
void layoutDecryptMessage(const uint8_t *msg, uint32_t len, const char *address) {
	...
	layoutDialogSwipe(&bmp_icon_info, NULL, "OK",
			address ? "Decrypted signed message" : "Decrypted message", str[0],
			str[1], str[2], str[3], NULL, NULL);
}
...
void layoutAddress(const char *address, const char *desc) {
	...
	const char **str = split_message((const uint8_t *) address, addrlen,
			rowlen);
	...
	oledDrawString(OLED_WIDTH - fontCharWidth('\x06') - 1, OLED_HEIGHT - 8,
			"\x06");
	oledDrawString(
			OLED_WIDTH - oledStringWidth(btnYes) - fontCharWidth('\x06') - 3,
			OLED_HEIGHT - 8, btnYes);
	oledInvert(OLED_WIDTH - oledStringWidth(btnYes) - fontCharWidth('\x06') - 4,
			OLED_HEIGHT - 9, OLED_WIDTH - 1, OLED_HEIGHT - 1);

	oledRefresh();
}

void layoutCurvePoint(const uint8_t *point, const char *name) {
	char hex[32 * 2 + 1];
	char desc[24] = "                        ";

	strlcpy(desc, name, sizeof(desc));
	data2hex(point, 1, desc + 22);
	data2hex(point + 1, 32, hex);
	const char **str = split_message((const uint8_t *) hex, 32 * 2, 16);
	layoutDialogSwipe(&bmp_icon_question, NULL, "Continue", desc, str[0],
			str[1], str[2], str[3], NULL, NULL);
}

void layoutBigInteger(const uint8_t *bigint, const char *name) {
	char hex[32 * 2 + 1];
	data2hex(bigint, 32, hex);
	const char **str = split_message((const uint8_t *) hex, 32 * 2, 16);
	layoutDialogSwipe(&bmp_icon_question, NULL, "Continue", name, str[0],
			str[1], str[2], str[3], NULL, NULL);
}
...
void layoutSignIdentity(const IdentityType *identity, const char *challenge) {
	...
	layoutDialogSwipe(&bmp_icon_question, "Cancel", "Confirm",
			...
			row_user[0] ? row_user : NULL, challenge,
			NULL,
			NULL);
}
...
void layoutU2FDialog(const char *verb, const char *appname,
	...
	layoutDialog(appicon, NULL, verb, NULL, verb, "U2F security key?", NULL,
			appname, NULL, NULL);
}
