...
bool protectPin(bool use_cached) {
	if (!storage.has_pin || storage.pin[0] == 0
			|| (use_cached && session_isPinCached())) {
		return true;
	}
	...
	while (wait > 0) {
		...
		layoutDialog(&bmp_icon_info, NULL, NULL, NULL, "Wrong PIN entered",
				NULL, "Please wait", secstr, "to continue ...", NULL);
		// wait one second
		usbSleep(1000);
		if (msg_tiny_id == MessageType_MessageType_Initialize) {
			protectAbortedByInitialize = true;
...
/*
 * returns
 * 		0, for not coerced PIN
 * 		1, for coerced PIN
 * 		2, for different PINs
 * 		3, for error
 */
Coerce_State protectEosPin(char *pin_return) {
	...
	if (strcmp(pin1, pin2) == 0) {
		strlcpy(pin_return, pin, 17);
		if (storage_isPinCorrect(pin1)) {
			return NOT_COERCED;
		} else {
			return COERCED;
		}
	} else {
		return PIN_MISSMATCH;
	}
}
...
bool protectPassphrase(void) {
	...
	layoutDialogSwipe(&bmp_icon_info, NULL, NULL, NULL, "Please enter your",
			"passphrase using", "the computer's", "keyboard.", NULL, NULL);
